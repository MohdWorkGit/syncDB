...
-- SUMMARY_ID is the target row's own generated id: insert.sql returns it via
-- RETURNING ... INTO :NEW_ID, and link.sql connects the link row to it. ORDER_ID
-- is the business key the engine uses for quarantine/history and your update.sql /
-- delete.sql match on it, so it is UNIQUE.
CREATE TABLE order_summary (
    ...
    CONSTRAINT order_summary_pk PRIMARY KEY (summary_id),
    CONSTRAINT order_summary_uq UNIQUE (order_id)
);

-- Link table (LINK_SQL_FILE): connects each newly inserted target row to a user.
-- link.sql writes one row here after every successful insert, referencing the new
-- SUMMARY_ID (the RETURNING value) plus the fixed SYNC_USER_ID / SYNC_USER_SECTION
-- from the environment — nothing from the source row. LINK_ID is the link table's
-- own generated id. The FK is ON DELETE CASCADE, so deleting the target row (via
-- delete.sql) also removes its link rows — link cleanup lives here, not in the
-- engine.
CREATE TABLE order_user_link (
    ...
    CONSTRAINT order_user_link_fk FOREIGN KEY (summary_id)
        REFERENCES order_summary (summary_id) ON DELETE CASCADE
);
...
-- one of INSERTED, UPDATED, DELETED, SKIPPED_BADROW, SKIPPED_QUARANTINED,
-- FAILED, LINK_FAILED; DETAIL carries the reason for the bad/failed ones.
